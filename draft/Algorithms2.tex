...
\[
\mathbf{X}_i = \text{FillNaN}(\mathbf{X}_i, 0) \tag{1}
\]
...
\[
\mathbf{patch}_i = \text{ExtractPatch}(\mathbf{coords}, \mathbf{raster\_files}) \tag{2}
\]
The patches are normalized to avoid division by zero:
\[
\mathbf{patch}_i = \frac{\mathbf{patch}_i}{\text{max}(\mathbf{patch}_i) + \epsilon} \tag{3}
\]
...
\[
\mathbf{h}_i^{\text{cnn}} = \text{Flatten}(\text{MaxPooling}(\text{Conv2D}(\mathbf{patch}_i))) \tag{6}
\]

\paragraph{MLP Branch:} The MLP branch processes the numerical input through fully connected layers:
\[
\mathbf{h}_i^{\text{mlp}} = \text{Dense}(\text{ReLU}(\mathbf{X}_i^{\text{mlp}})) \tag{7}
\]

\paragraph{GNN Branch:} The GNN branch processes the graph input using graph-based convolutions or attention mechanisms:
\[
\mathbf{h}_i^{\text{gnn}} = \text{Dense}(\text{ReLU}(\mathbf{G}_i)) \tag{8}
\]

\paragraph{Fusion Layer}
The outputs from all branches are concatenated:
\[
\mathbf{z}_i = \text{Concatenate}(\mathbf{h}_i^{\text{cnn}}, \mathbf{h}_i^{\text{mlp}}, \mathbf{h}_i^{\text{gnn}}) \tag{9}
\]
The concatenated vector is passed through dense layers:
\[
\mathbf{f}_i = \text{Dense}(\text{ReLU}(\mathbf{z}_i)) \tag{10}
\]
\[
\hat{y}_i = \text{Dense}(\mathbf{f}_i) \tag{11}
\]
...
\[
\mathbf{G}_{ij} = \exp\left( -\frac{d_{ij}}{\tau} \right), \tag{12}
\]
...
\[
\mathbf{A}_{ij} = \frac{\exp\left( \mathbf{e}_{ij} \right)}{\sum_{k=1}^{n} \exp\left( \mathbf{e}_{ik} \right)}, \tag{13}
\]
...
\[
\mathbf{h}_i^{\text{att}} = \sum_{j=1}^{n} \mathbf{A}_{ij} \mathbf{h}_j. \tag{14}
\]
...
\mathbf{V}_h = \mathbf{T}_i \mathbf{W}^{V}_h, \tag{15}
\]
\[
\text{Attn}_h(\mathbf{T}_i) = \text{softmax}\left( \frac{\mathbf{Q}_h \mathbf{K}_h^\top}{\sqrt{d_k}} \right) \mathbf{V}_h, \tag{16}
\]
\[
\text{MHA}(\mathbf{T}_i) = \big\|_{h=1}^{H} \text{Attn}_h(\mathbf{T}_i) \mathbf{W}^{O}. \tag{17}
\]
The multi-head attention outputs are combined, followed by residual connection and layer normalization:
\[
\tilde{\mathbf{T}}_i = \mathrm{LayerNorm}\left( \mathbf{T}_i + \text{Dropout}(\text{MHA}(\mathbf{T}_i)) \right). \tag{18}
\]
Finally, the flattened result is passed through dense layers for prediction:
\[
\mathbf{z}_i = \mathrm{Flatten}(\tilde{\mathbf{T}}_i) \in \mathbb{R}^{3d_p}, \tag{19}
\]
\[
\hat{y}_i = \mathbf{w}_3^\top \mathrm{ReLU}\left( \mathbf{W}_3 \mathrm{Dropout}\left( \mathrm{ReLU}\left( \mathbf{W}_4 \mathbf{z}_i + \mathbf{b}_4 \right) \right) \right) + \mathbf{b}_3. \tag{20}
\]
...
\[
\mathbf{z}_i = \text{Concatenate}(\mathbf{h}_i^{\text{cnn}}, \mathbf{h}_i^{\text{mlp}}, \mathbf{h}_i^{\text{gnn}}). \tag{21}
\]
This concatenated vector is passed through fully connected layers for prediction:
\[
\mathbf{f}_i = \text{Dense}(\text{ReLU}(\mathbf{z}_i)), \tag{22}
\]
\[
\hat{y}_i = \mathbf{W}_5 \mathbf{f}_i + \mathbf{b}_5. \tag{23}
\]
...
\[
\mathbf{z}_i = \text{Concatenate}(\mathbf{h}_i^{\text{gnn}}, \mathbf{h}_i^{\text{mlp}}). \tag{24}
\]

\paragraph{Decoder:}
The latent representation \(\mathbf{z}_i\) is decoded to reconstruct the target \(\hat{y}_i\):
\[
\mathbf{f}_i = \text{Dense}(\text{ReLU}(\mathbf{W}_3 \mathbf{z}_i + \mathbf{b}_3)), \tag{25}
\]
\[
\hat{y}_i = \mathbf{W}_4 \mathbf{f}_i + \mathbf{b}_4. \tag{26}
\]
...
\[
\mathbf{h}_i^{\text{cnn}} = \text{Flatten}(\text{MaxPool}(\text{Conv2D}(\tilde{\mathcal{P}}_i))). \tag{27}
\]

\paragraph{MLP Expert:}
The MLP expert processes tabular data:
\[
\mathbf{h}_i^{\text{mlp}} = \text{ReLU}(\mathbf{W}_1 \tilde{\mathbf{x}}_i + \mathbf{b}_1). \tag{28}
\]

\paragraph{GNN Expert:}
The GNN expert processes graph data:
\[
\mathbf{h}_i^{\text{gnn}} = \text{ReLU}(\mathbf{U}_1 \mathbf{g}_i + \mathbf{c}_1). \tag{29}
\]
...
\[
\text{Gate Input} = [\mathbf{h}_i^{\text{cnn}}, \mathbf{h}_i^{\text{mlp}}, \mathbf{h}_i^{\text{gnn}}], \tag{30}
\]
\[
\text{Gate Weights} = \text{Softmax}(\text{Dense}(\text{ReLU}(\text{Gate Input}))), \tag{31}
\]
\[
\hat{y}_i = \sum_{j=1}^{3} w_j \cdot y_j. \tag{32}
\]
...
\[
\text{Spatial Attention Map} = \sigma(\text{Conv1x1}(\tilde{\mathcal{P}}_i)), \tag{33}
\]
\[
X_{\text{cnn\_attn}} = X_{\text{cnn}} \cdot \text{Spatial Attention Map}. \tag{34}
\]
...
X_{\text{gnn\_attn}} = X_{\text{gnn}} \cdot \text{Feature Attention Map}. \tag{35}
\]

\paragraph{Gating Network:}
The outputs of the attention-modulated branches are combined:
\[
\text{Gate Input} = [\mathbf{h}_i^{\text{cnn}}, \mathbf{h}_i^{\text{mlp}}, \mathbf{h}_i^{\text{gnn}}], \tag{36}
\]
\[
\text{Gate Network} = \text{Dense}(\text{ReLU}(\text{Dense}(\text{Gate Input}))), \tag{37}
\]
\[
\hat{y}_i = \sum_{j=1}^{3} w_j \cdot y_j. \tag{38}
\]
...
\mathrm{RMSE}=\sqrt{\frac{1}{N}\sum_i (y_i-\hat{y}_i)^2}, \tag{39}
\]
\[
\mathrm{MAE}=\frac{1}{N}\sum_i |y_i-\hat{y}_i|,\qquad
\mathrm{SMAPE}=\frac{100}{N}\sum_{i=1}^N \frac{|\,\hat{y}_i-y_i\,|}{\tfrac{1}{2}\big(|y_i|+|\hat{y}_i|\big)+\varepsilon}, \tag{40}
\]
...
\[
\text{Importance}_{\text{CNN}} = R^2_{\text{baseline}} - R^2_{\text{permuted\_CNN}} \tag{44}
\]
\[
\text{Importance}_{\text{MLP}} = R^2_{\text{baseline}} - R^2_{\text{permuted\_MLP}} \tag{45}
\]
\[
\text{Importance}_{\text{GNN}} = R^2_{\text{baseline}} - R^2_{\text{permuted\_GNN}}. \tag{46}
\]
...
\[
\mathbf{X}_{\text{perturbed}}^{(i)} = \mathbf{X}_{\text{base}} + \epsilon, \tag{47}
\]
...
\[
\hat{y}_{\text{perturbed}} = f(\mathbf{X}_{\text{perturbed}}), \tag{48}
\]
...
\[
\hat{y} = \mathbf{w}^\top \mathbf{z} + b, \tag{49}
\]
...
\[
\hat{y}_i = \mathbf{w}_1^\top \mathbf{z}_i + b. \tag{50}
\]
