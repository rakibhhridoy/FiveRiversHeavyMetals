#!/usr/bin/env python3
"""
Optimized Transformer CNN GNN MLP Model for AlphaEarth Data
Enhanced architecture with better hyperparameters and training strategy
"""
...
from tensorflow.keras.layers import (
    ...
    MultiHeadAttention, LayerNormalization, Reshape, GlobalAveragePooling2D,
    BatchNormalization
)
...
BATCH_SIZE = 2  # Smaller batch for small dataset
TEST_SIZE = 0.2
RANDOM_STATE = 42
EPOCHS = 150
PATIENCE = 30
...
def load_data():
    ...
    print("\n" + "="*80)
    print("LOADING DATA - OPTIMIZED TRANSFORMER MODEL")
    print("="*80)
...
def build_optimized_transformer_model(cnn_shape, gnn_dim, mlp_dim):
    """
    Optimized Transformer CNN GNN MLP with:
    ...
    # First block: Conv + BatchNorm + Activation
    x_cnn = Conv2D(32, 3, padding='same', use_bias=False)(cnn_input)
    x_cnn = BatchNormalization()(x_cnn)
    x_cnn = tf.keras.activations.relu(x_cnn)
    x_cnn = Conv2D(32, 3, padding='same', use_bias=False)(x_cnn)
    x_cnn = BatchNormalization()(x_cnn)
    x_cnn = tf.keras.activations.relu(x_cnn)
    ...
    x_cnn = Conv2D(64, 3, padding='same', use_bias=False)(x_cnn)
    x_cnn = BatchNormalization()(x_cnn)
    x_cnn = tf.keras.activations.relu(x_cnn)
    x_cnn = Conv2D(64, 3, padding='same', use_bias=False)(x_cnn)
    x_cnn = BatchNormalization()(x_cnn)
    ...
    x_cnn = Conv2D(128, 3, padding='same', use_bias=False)(x_cnn)
    x_cnn = BatchNormalization()(x_cnn)
    x_cnn = tf.keras.activations.relu(x_cnn)
    x_cnn = GlobalAveragePooling2D()(x_cnn)

    # CNN Dense layers
    x_cnn = Dense(256, use_bias=False)(x_cnn)
    x_cnn = BatchNormalization()(x_cnn)
    x_cnn = tf.keras.activations.relu(x_cnn)
    x_cnn = Dropout(0.3)(x_cnn)
    x_cnn = Dense(128)(x_cnn)

    # =========== MLP BRANCH (Enhanced) ===========
    mlp_input = Input(shape=(mlp_dim,), name='mlp_input')

    x_mlp = Dense(128, use_bias=False)(mlp_input)
    x_mlp = BatchNormalization()(x_mlp)
    x_mlp = tf.keras.activations.relu(x_mlp)
    x_mlp = Dropout(0.3)(x_mlp)
    x_mlp = Dense(64, use_bias=False)(x_mlp)
    x_mlp = BatchNormalization()(x_mlp)
    x_mlp = tf.keras.activations.relu(x_mlp)
    x_mlp = Dropout(0.2)(x_mlp)
    x_mlp = Dense(32)(x_mlp)

    # =========== GNN BRANCH ===========
    gnn_input = Input(shape=(gnn_dim,), name='gnn_input')

    x_gnn = Dense(64, use_bias=False)(gnn_input)
    x_gnn = BatchNormalization()(x_gnn)
    x_gnn = tf.keras.activations.relu(x_gnn)
    x_gnn = Dropout(0.2)(x_gnn)
    x_gnn = Dense(32)(x_gnn)

    # =========== TRANSFORMER FUSION ===========
    # Reshape for attention: (batch, seq_len, features)
    x_cnn_seq = Reshape((16, 8))(x_cnn)  # CNN: 128 -> 16x8
    x_mlp_seq = Reshape((32, 1))(x_mlp)  # MLP: 32 -> 32x1
    x_gnn_seq = Reshape((32, 1))(x_gnn)  # GNN: 32 -> 32x1

    # Multi-head attention fusion
    # Query from CNN, Keys/Values from MLP+GNN
    key_value = Concatenate(axis=1)([x_mlp_seq, x_gnn_seq])  # (batch, 64, 1)
    ...
    # Layer normalization
    attn_output = LayerNormalization()(attn_output + x_cnn_seq)

    # Flatten and process
    x_fused = Flatten()(attn_output)

    # Dense layers for fusion
    x_fused = Dense(256, use_bias=False)(x_fused)
    x_fused = BatchNormalization()(x_fused)
    x_fused = tf.keras.activations.relu(x_fused)
    x_fused = Dropout(0.3)(x_fused)

    x_fused = Dense(128, use_bias=False)(x_fused)
    x_fused = BatchNormalization()(x_fused)
    x_fused = tf.keras.activations.relu(x_fused)
...
def train_with_augmentation(model, X_train_scaled, y_train, gnn_train_data,
                           raster_gen, train_idx, validation_split=0.2):
    """
    Train model with data augmentation and callbacks
    """
    print("Training with augmentation and learning rate scheduling...")
...
def main():
    print("\n" + "="*80)
    print("OPTIMIZED TRANSFORMER CNN GNN MLP FOR ALPHAEARTH")
    print("="*80)
    ...
    # Robust scaling for better convergence
    scaler = RobustScaler()
    ...
    print("\n" + "="*80)
    print("BUILDING OPTIMIZED TRANSFORMER MODEL")
    print("="*80)

    model = build_optimized_transformer_model(cnn_shape, gnn_test_data.shape[1], X_train_scaled.shape[1])
    ...
    print(f"\nOptimized Transformer CNN GNN MLP Results:")
    print(f"  R²:    {r2:.4f}")
    ...
    print("\nResults:")
    print(f"  89 features (unoptimized):    R² = 0.0780")
    print(f"  20 features (unoptimized):    R² = 0.7672")
    print(f"  20 features (OPTIMIZED):      R² = {r2:.4f} ✓")
    ...
    # Save results
    output_file = PROJECT_ROOT / "TRANSFORMER_OPTIMIZED_RESULTS.csv"
    results = pd.DataFrame({
        'Model': ['Transformer CNN GNN MLP (Optimized)'],
        'R²': [r2],
        'RMSE': [rmse],
        'MAE': [mae],
        'SMAPE': [smape]
