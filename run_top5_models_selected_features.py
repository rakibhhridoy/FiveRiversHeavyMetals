#!/usr/bin/env python3
"""
Run Top 5 Rainy Season Models with AlphaEarth - SELECTED FEATURES ONLY
Using top 20 features selected by SelectKBest (7 original + 13 AlphaEarth bands)
"""
...
RAINY_DATA_FILE = GIS_ROOT / "Option_B_RainyAE_SELECTED.csv"  # ← SELECTED FEATURES
...
def load_data():
    """Load AlphaEarth data with SELECTED FEATURES ONLY"""
    print("\n" + "="*80)
    print("LOADING DATA (SELECTED FEATURES)")
    print("="*80)
    ...
    print(f"  ✓ Using {len(feature_cols)} selected features (7 original + 13 AlphaEarth)")
    print(f"  Features: {X.shape[1]}")
    print(f"  Samples: {len(y)}")
    print(f"  Target range: {y.min():.2f} - {y.max():.2f}")

    # Show feature breakdown
    ae_features = [f for f in feature_cols if f.startswith('AE_')]
    orig_features = [f for f in feature_cols if not f.startswith('AE_')]
    print(f"\n  Feature breakdown:")
    print(f"    Original environmental: {len(orig_features)}")
    print(f"    AlphaEarth bands: {len(ae_features)}")
...
def main():
    print("\n" + "="*80)
    print("RUNNING TOP 5 RAINY SEASON MODELS WITH SELECTED FEATURES")
    print("20 Features: 7 Original + 13 AlphaEarth Bands")
    ...
    models_config = [
        {
            ...
            "builder": lambda: build_transformer_model(cnn_shape, gnn_test_data.shape[1], X_train_scaled.shape[1]),
            "has_cnn": True
        },
        {
            "name": "2. GNN MLP AE",
            "builder": lambda: build_gnn_mlp_ae_model(X_train_scaled.shape[1], gnn_test_data.shape[1]),
            "has_cnn": False
        },
        {
            "name": "3. CNN GNN MLP PG",
            "builder": lambda: build_cnn_gnn_mlp_pg_model(cnn_shape, gnn_test_data.shape[1], X_train_scaled.shape[1]),
            "has_cnn": True
        },
        {
            "name": "4. GNN MLP",
            "builder": lambda: build_gnn_mlp_model(X_train_scaled.shape[1], gnn_test_data.shape[1]),
            "has_cnn": False
        },
        {
            "name": "5. Stacked CNN GNN MLP",
            "builder": lambda: build_stacked_cnn_gnn_mlp_model(cnn_shape, gnn_test_data.shape[1], X_train_scaled.shape[1]),
            "has_cnn": True
        },
    ]
    ...
    # Summary
    print("\n" + "="*80)
    print("SUMMARY - ALL MODELS (SELECTED FEATURES)")
    print("="*80)
    ...
    # Save results
    output_file = PROJECT_ROOT / "TOP5_RAINY_SELECTED_FEATURES_RESULTS.csv"
    results_df.to_csv(output_file, index=False)
    print(f"\n✓ Results saved to: {output_file}")

    # Comparison
    print("\n" + "="*80)
    print("COMPARISON: ALL 89 FEATURES vs SELECTED 20 FEATURES")
    print("="*80)

    original_results = pd.read_csv(PROJECT_ROOT / "TOP5_RAINY_ALPHAEARTH_RESULTS.csv")

    comparison = pd.DataFrame({
        'Model': results_df['Model'],
        'R² (89 feat)': original_results['R²'].values,
        'R² (20 feat)': results_df['R²'].values,
        'Improvement': (results_df['R²'].values - original_results['R²'].values).round(4),
        'RMSE (89)': original_results['RMSE'].values,
        'RMSE (20)': results_df['RMSE'].values,
    })

    print("\n" + comparison.to_string(index=False))

    # Save comparison
    comparison_file = PROJECT_ROOT / "FEATURE_SELECTION_COMPARISON.csv"
